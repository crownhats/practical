...
public class Ring 
{
	...
	public void Election()
	{
		...
		{    
			...
			{
				System.out.println("Process " + process[old].id	+ " pass Election msg (" + process[old].id+ ") to" + process[newer].id);
				old = newer; 
			}
		...
		while (true) 
		{
			if (process[newer].act) 
			{
				System.out.println("Process " + process[old].id+ " pass (Coordinator is "+ coord+ ") message to process "+ process[newer].id);
				old = newer;
			}
			newer = (newer + 1) % TotalProcess;
			if(newer==coord)
